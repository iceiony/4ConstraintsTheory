...
//		}
	...
	frame->Show(TRUE);
		...
		// select solve mode
		wxMenuEvent solverMode (wxEVT_COMMAND_MENU_SELECTED, NewtonDemos::ID_SOLVER_MODE);
		frame->GetEventHandler()->ProcessEvent(solverMode);

		//select mesh collision
		wxMenuEvent meshMode (wxEVT_COMMAND_MENU_SELECTED, NewtonDemos::ID_SHOW_COLLISION_MESH+2);
		frame->GetEventHandler()->ProcessEvent(meshMode);
